...
import { AddressTypeString } from '../utils/address-helpers';
...
describe('AppController', () => {
  ...
  const sellerKeyPair = ECPair.fromWIF(
    'KwHninPcVBb64LQgEd9gKdwDap7V5EAcScXQC6rL16LXwgVk6YTr');
  const buyerKeyPair = ECPair.fromWIF(
    'L3jdqz2nPpY8KAYhiy9ofoxXPcX5n6Jijc65LKqARh44QFDWA2B3');
  ...
  describe('root', () => {
    it('sold and bought by tap root addresses should succeed', async () => {
      ...
      const sellerPsbtToSign = await appController.generateUnsignedSellerPsbt(
        orderInfo);
      console.log(sellerPsbtToSign);

      // 2. seller sign psbt
      const sellerPsbt = bitcoin.Psbt.fromHex(sellerPsbtToSign.psbtBase64);
      for (const signIndex of sellerPsbtToSign.signIndex) {
        sellerPsbt.signInput(signIndex.index,
          sellerKeyPairInfo.tweakedChildNode, [signIndex.sighashType]);
      }

      // 3. verify seller signed psbt
      const signedOrderInfo: SignedOrderInfo = {
        orderInfo: orderInfo,
        sellerPsbt: sellerPsbt.toHex(),
      };
      ...
      const buyerPsbtToSign = await appController.generateUnsignedBuyerPsbt(
        orderInfo);
      console.log(buyerPsbtToSign);

      // 5. buyer sign psbt
      const buyerPsbt = bitcoin.Psbt.fromHex(buyerPsbtToSign.psbtBase64);
      for (const signIndex of buyerPsbtToSign.signIndex) {
        buyerPsbt.signInput(signIndex.index, buyerKeyPairInfo.tweakedChildNode,
          [signIndex.sighashType]);
      }

      // 6. merge and extract tx
      signedOrderInfo.buyerPsbt = buyerPsbt.toHex();
      ...
      expect(rawTx).
        toEqual(
          '');
    });

    it('selled by tap root address and pay to p2pkh address should succeed',
      async () => {
        const orderInfo: OrderInfo = {
          atomicalId: '10581aa0fa9b5b1846131663f1513140e69530a9cc0e7ad1d831095443e7a851i0',
          unitPrice: 2,
          ...
          sellerAtomicals: [
            {
              txid: 'c3333d74e3ac5d9a9491732117e6ff0fa7e4f776e535d259a9b2c752415cf393',
              vout: 0,
              value: 1000,
              atomicals: ['10581aa0fa9b5b1846131663f1513140e69530a9cc0e7ad1d831095443e7a851i0'],
            }],
        ...
        const buyerInfo = {
          publicKey: '03980de2d22baf41dc1bfc8dd4a441e2998df1026c23f94eb18aaf5db2a4a3229e',
          address: 'bcrt1pqv42vjw50k0p29svd3l6er4s8r460evhylva0v3rmwmmnerl752qvpqq89',
          receiveAddress: getAddress(buyerKeyPair.publicKey, AddressTypeString.p2pkh_regtest),
          serviceFeeRate: 0.2,
          networkFeeRate: 30,
        };
        ...
        const buyerPsbt = bitcoin.Psbt.fromHex(buyerPsbtToSign.psbtBase64);
        for (const signIndex of buyerPsbtToSign.signIndex) {
          buyerPsbt.signInput(signIndex.index,
            buyerKeyPairInfo.tweakedChildNode, [signIndex.sighashType]);
        }

        // 6. merge and extract tx
        signedOrderInfo.buyerPsbt = buyerPsbt.toHex();
        const rawTx = appController.extractSellerBuyerTxFromPsbt(
          signedOrderInfo);
        const tx = bitcoin.Transaction.fromHex(rawTx);
    ...
    it('parse psbt', async () => {
      ...
      const sellerPsbt = bitcoin.Psbt.fromHex(psbt,
        { network: bitcoin.networks.testnet });
    });
  });
  it('test', async () => {
    const p2pkh = getAddress(buyerKeyPair.publicKey, AddressTypeString.p2pkh_regtest);
    console.log(p2pkh);

    const p2tr = getAddress(buyerKeyPair.publicKey, AddressTypeString.p2tr_regtest);
    console.log(p2tr);
  });
});

function genOrderInfo(
  sellerPublicKey: string, sellerAddress: string, sellerReceiveAddress: string,
  atomicals: Atomical[], unitPrice: number) {
  const orderInfo: OrderInfo = {
    atomicalId: '10581aa0fa9b5b1846131663f1513140e69530a9cc0e7ad1d831095443e7a851i0',
    unitPrice: 2,
    platformReceiveAddress: 'bcrt1p9mcphvr9xa37xxlgas2ps9e6yakupj6jg58g3j94pdwmm4wwlj7q4zd28a',
...
function getAddress(publicKey: Buffer, addressType: AddressTypeString): string {
  switch (addressType) {
    case AddressTypeString.p2pkh:
    case AddressTypeString.p2pkh_regtest:
    case AddressTypeString.p2pkh_testnet: {
      const { address } = bitcoin.payments.p2pkh(
        { pubkey: publicKey, network: NETWORK });
      return address;
    }
    case AddressTypeString.p2sh:
    case AddressTypeString.p2sh_regtest:
    ...
    case AddressTypeString.p2wpkh_regtest:
    ...
    case AddressTypeString.p2tr: {
      const childNodeXOnlyPubkeyPrimary = toXOnly(publicKey);
      const { address } = bitcoin.payments.p2tr(
        { internalPubkey: childNodeXOnlyPubkeyPrimary, network: NETWORK });
      return address;
    }
    default: {
      throw new Error(`address type ${addressType} is not supported`);
    }
  }
}
